        ...
        */
        private string PrintRecursive(IEmployee employeeToPrint, int hierarchyLevel)
        {
            ...
            else if(employeeToPrint is Manager<IEmployee>)
            {
                ...
                {
                    // Create a new, sorted collection of the Manager's employees
                    var sortedManagerEmployees = managerEmployees
                        .Select( x => x )           // Select everything from the 'managerEmployees' collection
                        .OrderBy( x => x.Name );    // Sort the new collection based on the 'Name' property
